...
function FailWiseApp() {
  ...
  useEffect(() => {
    ...
    const steps = [
      ...
      () => setSelectedNodeId('mcp-tool'),
      () => {
        setDraftFailureMode('mcp_timeout');
        setActiveFailureMode('mcp_timeout');
        setSelectedNodeId('mcp-tool');
        window.scrollTo({ top: 430, behavior: 'smooth' });
      },
      () => setSelectedNodeId('cache-fallback'),
      () => setSelectedNodeId('final-answer'),
      () => {
        setRunSource('deepseek_live');
        setDeepSeekProbe({
          ok: false,
          model: 'deepseek-chat',
          latencyMs: 1240,
          failureCase: 'LLM_PROVIDER_FAILURE',
          errorMessage: 'Simulated provider authentication failure for demo recording.',
        });
        setSelectedNodeId('llm-gateway');
        window.scrollTo({ top: 430, behavior: 'smooth' });
      },
      () => setSelectedNodeId('rule-fallback'),
      () => {
        window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
      },
      () => {
        setRunSource('simulation');
        setDraftFailureMode('cache_miss');
        setActiveFailureMode('cache_miss');
        setDeepSeekProbe(null);
        setSelectedNodeId('safe-refusal');
        window.scrollTo({ top: 430, behavior: 'smooth' });
      },
      () => {
        window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' });
    ...
    const schedule = [1000, 21000, 26000, 34000, 42000, 50000, 56000, 64000, 70000, 76000, 82000, 88000];
    const timers = steps.map((step, index) => window.setTimeout(step, schedule[index]));
